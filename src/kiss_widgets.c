	...
	int x, int y, int h)
{
	...
	kiss_makerect(&vscrollbar->sliderrect, x, y + vscrollbar->uprect.h +
		kiss_edge/2, vscrollbar->vslider.w, vscrollbar->vslider.h);
...
static void vnewpos(kiss_vscrollbar *vscrollbar, double step)
{
...
int kiss_vscrollbar_event(kiss_vscrollbar *vscrollbar, Input* input, InputMouse* inputMouse)
{
	if (!vscrollbar) return 0;
	if (!(inputMouse->isMouseClicked && inputMouse->mouse[SDL_BUTTON_LEFT] == 1)) {
		vscrollbar->upclicked = 0;
		vscrollbar->downclicked = 0;
		vscrollbar->lasttick = 0;
	} else if (vscrollbar->upclicked && kiss_getticks() >
		vscrollbar->lasttick + kiss_click_interval) {
		vnewpos(vscrollbar, -vscrollbar->step);
		return 1;
	...
	/*if (!vscrollbar->focus && (!vscrollbar->wdw ||
		...
		return 0;*/
	if (inputMouse->isMouseClicked && kiss_pointinrect(inputMouse->positionMouse.x,inputMouse->positionMouse.y,&vscrollbar->uprect) && vscrollbar->step > 0.000001) {
		if (vscrollbar->fraction > 0.000001) {
	...
	} else if (inputMouse->isMouseClicked &&
		kiss_pointinrect(inputMouse->positionMouse.x, inputMouse->positionMouse.y,&vscrollbar->downrect) && vscrollbar->step > 0.000001) {
		...
		vscrollbar->lasttick = kiss_getticks() -
			kiss_click_interval - 1;
	} else if (inputMouse->isMouseClicked && kiss_pointinrect(inputMouse->positionMouse.x,inputMouse->positionMouse.y, &vscrollbar->sliderrect) && vscrollbar->step > 0.000001) {
		if (vscrollbar->wdw) vscrollbar->wdw->focus = 0;
		vscrollbar->focus = 1;
		vscrollbar->sliderclicked = 1;
	} else if (inputMouse->isMouseUp) {
		vscrollbar->upclicked = 0;
		vscrollbar->downclicked = 0;
		vscrollbar->lasttick = 0;
		...
		vscrollbar->focus = 0;
		vscrollbar->sliderclicked = 0;
	} else if (inputMouse->isMouseMotion &&
		(inputMouse->isMouseClicked && inputMouse->mouse[SDL_BUTTON_LEFT] == 1) &&
		vscrollbar->sliderclicked) {
		vnewpos(vscrollbar, 1. *inputMouse->positionMouse.yrel /
			vscrollbar->maxpos);
        inputMouse->isMouseMotion = false;
		return 1;
...
int kiss_button_event(kiss_button *button, Input* input, InputMouse* inputMouse)
{
	if (!button) return 0;
	if (inputMouse->isMouseClicked &&
		...
		button->active = 0;
		return 1;
	} else if (kiss_pointinrect(inputMouse->positionMouse.x,inputMouse->positionMouse.y,
		&button->rect)) {
		button->prelight = 1;
	} else if (!kiss_pointinrect(inputMouse->positionMouse.x,inputMouse->positionMouse.y,
		&button->rect)) {
...
int kiss_entry_event(kiss_entry *entry, Input* input, InputMouse* inputMouse)
{
	if (!entry)  return 0;
...
int kiss_textbox_event(kiss_textbox *textbox, Input* input, InputMouse* inputMouse)
{
	int texty, textmaxy, numoflines;

	if (!textbox || !textbox->array || !textbox->array->length)
		return 0;
	if (inputMouse->isMouseClicked &&
		kiss_pointinrect(inputMouse->positionMouse.x, inputMouse->positionMouse.y,
		&textbox->textrect)) {
		numoflines = textbox_numoflines(textbox);
		texty = inputMouse->positionMouse.y - textbox->textrect.y;
...
int kiss_textbox_draw(kiss_textbox *textbox, SDL_Renderer *renderer)
{
	...
	for (i = 0; i < numoflines; i++) {
        text = (char *) kiss_array_data(textbox->array,textbox->firstline + i);
        maxLength = kiss_maxlength(textbox->font,textbox->textwidth,text, NULL);
		kiss_string_copy(buf,maxLength ,text, NULL);
		kiss_rendertextSaveWidth(renderer, buf, textbox->textrect.x,
        textbox->textrect.y + i * textbox->font.lineheight +
        textbox->font.spacing / 2, textbox->font,
        textbox->textcolor,&textbox->array->lengthText[textbox->firstline + i],text);

        if(textbox->array->lengthText[textbox->firstline + i]>textbox->textwidth)
        {
            kiss_textbox_handleToLongText(textbox, textbox->firstline + i,text,maxLength);
        }
...
void kiss_textbox_handleToLongText(kiss_textbox* textbox, int index, char* text,int maxLength)
{
      char *buf = malloc(KISS_MAX_LENGTH);
      kiss_string_copy(buf,maxLength ,text, NULL);
      char* substr = malloc(KISS_MAX_LENGTH);
      strcpy(substr, text+strlen(buf));
      kiss_array_assign(textbox->array,index,0,buf);

      if(index + 1 >= textbox->array->length)
        kiss_array_appendstring(textbox->array,0,substr,NULL);
      else
        kiss_array_insert(textbox->array,index+1,0,substr);
}
