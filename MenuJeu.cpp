...
MenuJeu::MenuJeu(Input* input,InputMouse* inputMouse,SDL_Window* screen) : Menu(input,inputMouse,screen)
{
    ...
    {
        listeJoystick.push_back(SDL_JoystickOpen(i));
    }

    std::vector<SDL_JoystickID> joystickAvailable;

    for(unsigned int i = 0; i<listeJoystick.size();i++)
    {
        joystickAvailable.push_back(SDL_JoystickInstanceID(listeJoystick.at(i)));
        Jeu::initJoystick(joystickAvailable.at(i));
    }

    for(unsigned int i = 0;i<listeBomber.size();i++)
    {
        if(listeBomber.at(i)->getPlayerKeyboard().pad)
        {
           if(joystickAvailable.size() > 0)
           {
               listeBomber.at(i)->setJoystickID(joystickAvailable.at(0));
               joystickAvailable.erase(joystickAvailable.begin());
           }
        }
    }
...
MenuJeu::~MenuJeu()
{
    ...
    for(unsigned int i = 0;i<listeJoystick.size();i++)
    {
        SDL_JoystickClose(listeJoystick.at(i));
        listeJoystick.erase(listeJoystick.begin() + i);
        i--;
    }
