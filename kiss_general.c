...
int kiss_array_new(kiss_array *a)
{
	...
	a->lengthText = (int **) malloc(KISS_MIN_LENGTH * sizeof(int *));
...
int kiss_array_append(kiss_array *a, int id, void *data)
{
	...
	if (a->length >= a->size) {
		...
		a->data = (void **) realloc(a->data,
			a->size * sizeof(void *));
        a->lengthText = (int **) realloc(a->lengthText,
			a->size * sizeof(int *));
...
int kiss_array_remove(kiss_array *a, int index)
{
	...
	for (i = index; i < a->length - 1; i++) {
		a->data[i] = a->data[i + 1];
		a->id[i] = a->id[i + 1];
		a->lengthText[i] = a->lengthText[i + 1];
	}
	a->data[a->length - 1] = NULL;
	a->id[a->length - 1] = 0;
	a->lengthText[a->length - 1] = 0;
	--a->length;
	return 0;
}
