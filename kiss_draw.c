...
int kiss_rendertextSaveWidth(SDL_Renderer *renderer, char *text, int x, int y,
	kiss_font font, SDL_Color color, int* lengthText, char* textOriginal)
{
	SDL_Surface *surface, *surfaceOriginale;
	kiss_image image, imageOriginale;

	if (!text || !renderer || !font.font) return -1;
	surface = TTF_RenderUTF8_Blended(font.font, text, color);
	surfaceOriginale = TTF_RenderUTF8_Blended(font.font, textOriginal, color);
	image.image = SDL_CreateTextureFromSurface(renderer, surface);
	imageOriginale.image = SDL_CreateTextureFromSurface(renderer, surfaceOriginale);
	SDL_QueryTexture(image.image, NULL, NULL, &image.w, &image.h);
	SDL_QueryTexture(imageOriginale.image, NULL, NULL, lengthText, NULL);
	if (surface) SDL_FreeSurface(surface);
	if (surfaceOriginale) SDL_FreeSurface(surfaceOriginale);
	kiss_renderimage(renderer, image, x, y, NULL);
	SDL_DestroyTexture(image.image);
	SDL_DestroyTexture(imageOriginale.image);
	return 0;
}
...
void kiss_init(kiss_array *a,SDL_Renderer* renderer)
{
	int r = 0;
