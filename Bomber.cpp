...
Bomber::Bomber() : Personnage()
{
    ...
    if(player==1)
    {
        playerKeyboard = {SDL_SCANCODE_W,SDL_SCANCODE_S,SDL_SCANCODE_D,SDL_SCANCODE_A,SDL_SCANCODE_SPACE, false};
    ...
    {
        playerKeyboard = {SDL_SCANCODE_UP,SDL_SCANCODE_DOWN,SDL_SCANCODE_RIGHT,SDL_SCANCODE_LEFT,SDL_SCANCODE_RCTRL, false};
...
{
    bool goUp = playerKeyboard.pad && Jeu::getJoystick()[joystickID] != nullptr ? Jeu::getJoystick()[joystickID]->up : in->key[playerKeyboard.up];
    bool goDown = playerKeyboard.pad && Jeu::getJoystick()[joystickID] != nullptr ? Jeu::getJoystick()[joystickID]->down : in->key[playerKeyboard.down];
    bool goLeft = playerKeyboard.pad && Jeu::getJoystick()[joystickID] != nullptr ? Jeu::getJoystick()[joystickID]->left : in->key[playerKeyboard.left];
    bool goRight = playerKeyboard.pad && Jeu::getJoystick()[joystickID] != nullptr ? Jeu::getJoystick()[joystickID]->right : in->key[playerKeyboard.right];
    bool goBomb = playerKeyboard.pad && Jeu::getJoystick()[joystickID] != nullptr ? Jeu::getJoystick()[joystickID]->bomb : in->key[playerKeyboard.bomb];
...
void Bomber::setJoystickID(SDL_JoystickID id)
{
    joystickID = id;
}

SDL_JoystickID Bomber::getJoystickID()
{
    return joystickID;
}
